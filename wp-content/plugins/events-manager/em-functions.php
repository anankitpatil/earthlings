...
function em_get_search_form_defaults($args = array())
{
    ...
    if (!empty($search_args['css'])) {
        $search_args['main_classes'][] = 'css-search';
    }
    if (!empty($search_args['search_term'])) {
        $search_args['main_classes'][] = 'has-search-term';
    }
    if (!empty($search_args['search_geo'])) {
        $search_args['main_classes'][] = 'has-search-geo';
    }
    ...
    if (isset($_REQUEST['geo'])) {
        $args['geo'] = $_REQUEST['geo'];
    } //if geo search string requested, use that for search form
    if (isset($_REQUEST['near'])) {
        $args['near'] = wp_unslash($_REQUEST['near']);
    } //if geo search string requested, use that for search form
    ...
    } //if state requested, use that for searching
    if (isset($_REQUEST['country'])) {
        $args['country'] = wp_unslash($_REQUEST['country']);
    } //if country requested, use that for searching
    if (isset($_REQUEST['region'])) {
        $args['region'] = wp_unslash($_REQUEST['region']);
    } //if region requested, use that for searching
    if (isset($_REQUEST['state'])) {
        $args['state'] = wp_unslash($_REQUEST['state']);
    } //if state requested, use that for searching
    if (isset($_REQUEST['town'])) {
        $args['town'] = wp_unslash($_REQUEST['town']);
    } //if state requested, use that for searching
    if (isset($_REQUEST['near_unit'])) {
        $args['near_unit'] = $_REQUEST['near_unit'];
    } //if state requested, use that for searching
...
function em_options_input_text($title, $name, $description = '', $default = '')
{
    ...
    ?>
	<tr valign="top" id='<?php echo esc_attr($name);
    ?>_row'>
		<th scope="row"><?php echo esc_html($title);
    ?></th>
	    <td>
			<input name="<?php echo esc_attr($name) ?>" type="text" id="<?php echo esc_attr($name) ?>" style="width: 95%" value="<?php echo esc_attr(get_option($name, $default), ENT_QUOTES);
    ?>" size="45" />
                ...
                if ($translate) {
                    ...
                    foreach (EM_ML::get_langs() as $lang => $lang_name) {
                        if ($lang != EM_ML::$wplang) {
                            ?>
							<tr>
								<td class="lang"><?php echo $lang_name;
                            ?></td>
								<td class="lang-text"><input name="<?php echo esc_attr($name) ?>_ml[<?php echo $lang ?>]" type="text" id="<?php echo esc_attr($name.'_'.$lang) ?>" style="width: 100%" value="<?php echo esc_attr(EM_ML::get_option($name, $lang, false), ENT_QUOTES);
                            ?>" size="45" /></td>
							</tr>
							<?php
...
function em_options_input_password($title, $name, $description = '')
{
    ...
    ?></th>
	    <td>
			<input name="<?php echo esc_attr($name) ?>" type="password" id="<?php echo esc_attr($title) ?>" style="width: 95%" value="<?php echo esc_attr(get_option($name));
    ?>" size="45" /><br />
			<em><?php echo $description;
    ?></em>
		</td>
...
function em_options_textarea($title, $name, $description = '')
{
    ...
    ?></th>
			<td>
				<textarea name="<?php echo esc_attr($name) ?>" id="<?php echo esc_attr($name) ?>" rows="6" cols="60"><?php echo esc_attr(get_option($name), ENT_QUOTES);
    ?></textarea>
		    	<?php if ($translate): ?><span class="em-translatable  dashicons dashicons-admin-site"></span><?php endif;
    ?>
		    	<br />
                        ...
                        foreach (EM_ML::get_langs() as $lang => $lang_name) {
                            if ($lang != EM_ML::$wplang) {
								...
								<tr>
									<td class="lang"><?php echo $lang_name;
                                ?></td>
									<td class="lang-text"><textarea name="<?php echo esc_attr($name) ?>_ml[<?php echo $lang ?>]" id="<?php echo esc_attr($name.'_'.$lang) ?>" style="width: 100%" size="45"><?php echo esc_attr(EM_ML::get_option($name, $lang, false), ENT_QUOTES);
                                ?></textarea></td>
								</tr>
								<?php
...
function em_options_radio($name, $options, $title = '')
{
    ...
    ?>" name="<?php echo esc_attr($name) ?>" type="radio" value="<?php echo esc_attr($value);
    ?>" <?php if ($option == $value) {
...
function em_options_radio_binary($title, $name, $description = '', $option_names = '', $trigger = '')
{
    if (empty($option_names)) {
        $option_names = array(0 => __('No', 'events-manager'), 1 => __('Yes', 'events-manager'));
    }
    ...
    ?>
   	<tr valign="top" id='<?php echo $name;
    ?>_row'>
   		<th scope="row"><?php echo esc_html($title);
    ?></th>
   		<td>
   			<?php echo $option_names[1];
    ?> <input id="<?php echo esc_attr($name) ?>_yes" name="<?php echo esc_attr($name) ?>" type="radio" value="1" <?php if ($list_events_page) {
    echo "checked='checked'";
}
    echo $trigger_att;
    ?> />&nbsp;&nbsp;&nbsp;
			<?php echo $option_names[0];
    ?> <input  id="<?php echo esc_attr($name) ?>_no" name="<?php echo esc_attr($name) ?>" type="radio" value="0" <?php if (!$list_events_page) {
    echo "checked='checked'";
}
    echo $trigger_att;
    ?> />
			<br/><em><?php echo $description;
    ?></em>
		</td>
   	</tr>
	<?php
