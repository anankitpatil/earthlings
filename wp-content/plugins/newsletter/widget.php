...
class NewsletterWidget extends WP_Widget {
    ...
    static function get_widget_form() {
        ...
        $form .= '<p class="newsletter-para text-center">Subscribe to the newsletter to receive related articles from Earthlings about Access Consciousness, Visual Art Therapy and Clinical Hypnotherapy.</p>';

        $form .= '<form class="newsletter-form" action="' . home_url('/') . '?na=s" onsubmit="return newsletter_check(this)" method="post">';
        // Referrer
        ...
        $form .= '<div class="form-group"><input class="newsletter-email form-control" type="email" required name="ne" value="' . esc_attr($options_profile['email']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></div>';
