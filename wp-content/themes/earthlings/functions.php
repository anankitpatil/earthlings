...
/**
 * Add events to default categories
 */
...
function xsbf_setup() {
	...
	register_nav_menus( array(
		'primary' 	=> __( 'Header Menu', 'earthlings' ),
		'footer' 	=> __( 'Footer Menu', 'earthlings' ),
	) );
	...
	load_theme_textdomain( 'earthlings', get_template_directory() . '/languages' );
...
function xsbf_widgets_init() {
	...
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'earthlings' ),
		'id'            => 'sidebar-1',
		'description' 	=> __( 'Main sidebar (right or left)', 'earthlings' ),
	...
	register_sidebar( array(
		'name' 			=> __( 'Footer', 'earthlings' ),
		'id' 			=> 'sidebar-2',
		'description' 	=> __( 'Optional site footer widgets. Add 1-4 widgets here to display in columns.', 'earthlings' ),
		'before_widget' => '<aside id="%1$s" class="widget col-md-4 clearfix %2$s">',
		'after_widget' 	=> "</aside>",
	...
	register_sidebar( array(
		'name' 			=> __( 'Page Top', 'earthlings' ),
		'id' 			=> 'sidebar-3',
		'description' 	=> __( 'Optional section after the header. This is a single column area that spans the full width of the page.', 'earthlings' ),
	...
	register_sidebar( array(
		'name' 			=> __( 'Page Bottom', 'earthlings' ),
		'id' 			=> 'sidebar-4',
		'description' 	=> __( 'Optional section before the footer. This is a single column area that spans the full width of the page.', 'earthlings' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s clearfix"><div class="container">',
		'before_title' 	=> '<h2 class="widget-title">',
		'after_title' 	=> '</h2>',
		'after_widget' 	=> '</div><!-- container --></aside>',
	...
	register_sidebar( array(
		'name' 			=> __( 'Home Page', 'earthlings' ),
		'id' 			=> 'sidebar-5',
		'description' 	=> __( 'Optional section that displays only on the home page. It appears whether the home page is static or the blog. This is a single column area that spans the full width of the page.', 'earthlings' ),
...
function xsbf_load_css() {
	...
	// Google fonts
	wp_register_style('google-fonts-suravaram', 'https://fonts.googleapis.com/css?family=Suravaram|Muli' );
	wp_enqueue_style( 'google-fonts-suravaram');

	// Load animate.css
	wp_register_style('animate', get_template_directory_uri() . '/css/animate.min.css', array( 'bootstrap' ), '3.5.0', 'all' );
	...
	wp_enqueue_style( 'earthlings', get_stylesheet_uri() . '?time=' . time() );
...
function xsbf_load_js() {
	...
	// Theme js
	wp_enqueue_script( 'theme', get_template_directory_uri() . '/js/theme.js?time=' . time(), array('jquery'), '20160303', true );

	// Other external JS
	wp_enqueue_script( 'wow', get_template_directory_uri() . '/js/wow.min.js', array('jquery'), '1.1.2', true );
	wp_enqueue_script( 'svg', get_template_directory_uri() . '/js/jquery.svg.min.js', array('jquery'), '1.5.0', true );
...
/* Add events to category pages */
function add_custom_types_to_tax( $query ) {
	if( is_category( 'Community Events' ) && empty( $query->query_vars['suppress_filters'] ) ) {
		$query->set( 'post_type', 'event' );
		$query->set( 'category_name', 'Community Events' );
		return $query;
	}
}
