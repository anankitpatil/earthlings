...
function xsbf_get_the_excerpt( $excerpt ) {
	...
	if ( ! is_attachment() ) {
		...
		$excerpt .= '<a class="read-more" href="'. get_permalink( get_the_ID() ) . '">' . __( 'Read More', 'earthlings' ) . '</a>';
	}
	return $excerpt;
}
endif; // end ! function_exists
...
function xsbf_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'earthlings' ); ?> <?php comment_author_link(); ?>
			<?php edit_comment_link( __( '<span class="glyphicon glyphicon-edit"></span> Edit', 'earthlings' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'earthlings' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author -->

				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'earthlings' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
					<?php edit_comment_link( __( 'Edit', 'earthlings' ), '<span class="edit-link"><span class="glyphicon glyphicon-edit"></span> ', '</span>' ); ?>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'earthlings' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
