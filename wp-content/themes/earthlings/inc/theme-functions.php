...
function xsbf_add_custom_box() {
    ...
    foreach ( $screens as $screen ) {
        add_meta_box(
            'xsbf_options',
            __( 'Additional Post / Page Options', 'earthlings' ),
...
function xsbf_inner_custom_box( $post ) {
	...
	echo '<label for="xsbf_post_subtitle">';
	   _e( "Subtitle (displays under the title in the content header)", 'earthlings' );
	echo '</label> ';
	...
	if ( 'post' == $post->post_type ) {
		...
		echo '<label for="xsbf_post_template">';
		   _e( "Display post full-width (no sidebar)?", 'earthlings' );
		echo '</label> ';
	} //endif 'post'
}
endif; // end ! function_exists
